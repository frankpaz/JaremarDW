...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Ventas", "stg", "stg", "factVentasLineas", "int", "factVentas", "Incremental",
    )
    ...
    try:
        ...
        if nuevo_max is not None:
            ...
            cur.execute(
                ...
                "Ventas", nueva_fecha_hora, "Incremental",
            )
            print(f"Watermark 'Ventas' actualizado a {nueva_fecha_hora}")
        conn.commit()
