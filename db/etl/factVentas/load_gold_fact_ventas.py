"""
Gold: [int].factVentas -> dw.factVentas (SCD Tipo 1, nombres de negocio,
FKs a dimEmpresas/dimProducto).

Todo ocurre dentro de JAREMAR -- el trabajo real lo hace
dw.usp_MergeFactVentas; este script solo orquesta el ciclo de control
(registro de proceso, log de corrida, watermark).

Uso:
    python db/etl/factVentas/load_gold_fact_ventas.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Ventas", "int", "int", "factVentas", "dw", "factVentas", "Incremental",
    )
...
def main() -> int:
    ...
    try:
        cur.execute(
            "EXEC dw.usp_MergeFactVentas @RunId = ?, @UltimoWatermark = ?, @Reconciliar = ?",
