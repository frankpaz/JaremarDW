"""
Silver: stg.factMeteoDaily -> [int].factMeteoDaily (tipado, dedup por
Site+MeasuredDate).

stg.factMeteoDaily es una serie diaria curada manualmente (fuente NASA
POWER), no un extract_*.py de este repo -- este pipeline arranca directo en
Silver. A diferencia de los demas facts del dominio Solar (entidades ABP),
este no trae Id/IsDeleted/CreationTime -- se trata con el patron FULL +
SCD Tipo 1 de las dimensiones (no incremental por watermark). Todo ocurre
dentro de JAREMAR -- el trabajo real lo hace [int].usp_MergeFactMeteoDaily;
este script solo orquesta el ciclo de control (registro de proceso, log de
corrida, watermark).

Uso:
    python db/etl/factMeteoDaily/load_silver_fact_meteo_daily.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Solar", "stg", "stg", "factMeteoDaily", "int", "factMeteoDaily", "FULL",
    )
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC [int].usp_MergeFactMeteoDaily @RunId = ?", run_id)
