"""
Gold: [int].factMeteoDaily -> dw.factMeteoDaily (SCD Tipo 1).

Todo ocurre dentro de JAREMAR -- el trabajo real lo hace
dw.usp_MergeFactMeteoDaily; este script solo orquesta el ciclo de control
(registro de proceso, log de corrida, watermark).

Uso:
    python db/etl/factMeteoDaily/load_gold_fact_meteo_daily.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Solar", "int", "int", "factMeteoDaily", "dw", "factMeteoDaily", "FULL",
    )
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC dw.usp_MergeFactMeteoDaily @RunId = ?", run_id)
