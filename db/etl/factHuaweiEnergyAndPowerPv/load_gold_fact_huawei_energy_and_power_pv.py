"""
Gold: [int].factHuaweiEnergyAndPowerPv -> dw.factHuaweiEnergyAndPowerPv
(INCREMENTAL).

Requiere que dw.dimHuaweiDevices ya este cargado
(dw.usp_MergeFactHuaweiEnergyAndPowerPv resuelve HuaweiDeviceKey por
...
Uso:
    python db/etl/factHuaweiEnergyAndPowerPv/load_gold_fact_huawei_energy_and_power_pv.py [--env-file .env]
"""
import argparse
import sys
...
PROCESO = "HuaweiEnergyAndPowerPv_Gold"
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Solar", "int", "int", "factHuaweiEnergyAndPowerPv", "dw", "factHuaweiEnergyAndPowerPv", "Incremental",
    )
...
def main() -> int:
    ...
    try:
        cur.execute(
            "EXEC dw.usp_MergeFactHuaweiEnergyAndPowerPv @RunId = ?, @UltimoWatermark = ?",
