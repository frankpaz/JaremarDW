"""
Silver: stg.factHuaweiEnergyAndPowerPv -> [int].factHuaweiEnergyAndPowerPv
(tipado, dedup por Id, INCREMENTAL).

stg.factHuaweiEnergyAndPowerPv lo carga un proceso externo al dominio solar
(API Huawei FusionSolar), no un extract_*.py de este repo -- este pipeline
...
LastModificationTime posteriores al ultimo watermark del proceso. El KPI
diario llega como JSON (KpiData) y se parsea a columnas tipadas en
[int].usp_MergeFactHuaweiEnergyAndPowerPv.

Todo ocurre dentro de JAREMAR -- este script solo orquesta el ciclo de
control (registro de proceso, log de corrida, watermark).

Uso:
    python db/etl/factHuaweiEnergyAndPowerPv/load_silver_fact_huawei_energy_and_power_pv.py [--env-file .env]
"""
import argparse
import sys
...
PROCESO = "HuaweiEnergyAndPowerPv_Silver"
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Solar", "stg", "stg", "factHuaweiEnergyAndPowerPv", "int", "factHuaweiEnergyAndPowerPv", "Incremental",
    )
...
def main() -> int:
    ...
    try:
        cur.execute(
            "EXEC [int].usp_MergeFactHuaweiEnergyAndPowerPv @RunId = ?, @UltimoWatermark = ?",
