"""
Silver: stg.factSmaPower15Minutes -> [int].factSmaPower15Minutes (tipado,
dedup por Id, INCREMENTAL).

stg.factSmaPower15Minutes lo carga un proceso externo al dominio solar (API
SMA), no un extract_*.py de este repo -- este pipeline arranca directo en
Silver. Es un fact table de serie de tiempo (carga INCREMENTAL, no FULL): el
merge solo procesa filas con CreationTime/LastModificationTime posteriores
al ultimo watermark del proceso, leido de dbo.usp_Etl_WatermarkObtener y
actualizado al final con el maximo CreationTime/LastModificationTime visto
en el batch (no con la hora actual).

Todo ocurre dentro de JAREMAR -- el trabajo real lo hace
[int].usp_MergeFactSmaPower15Minutes; este script solo orquesta el ciclo de
control (registro de proceso, log de corrida, watermark).

Uso:
    python db/etl/factSmaPower15Minutes/load_silver_fact_sma_power_15minutes.py [--env-file .env]
"""
import argparse
import sys
...
PROCESO = "SmaPower15Minutes_Silver"
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Solar", "stg", "stg", "factSmaPower15Minutes", "int", "factSmaPower15Minutes", "Incremental",
    )
...
def main() -> int:
    ...
    try:
        cur.execute(
            "EXEC [int].usp_MergeFactSmaPower15Minutes @RunId = ?, @UltimoWatermark = ?",
