"""
Gold: [int].factSmaPower15Minutes -> dw.factSmaPower15Minutes (INCREMENTAL).

Requiere que dw.dimSmaPlants y dw.dimSmaDevices ya esten cargados
(dw.usp_MergeFactSmaPower15Minutes resuelve SmaPlantaKey/SmaDeviceKey contra
...
Uso:
    python db/etl/factSmaPower15Minutes/load_gold_fact_sma_power_15minutes.py [--env-file .env]
"""
import argparse
import sys
...
PROCESO = "SmaPower15Minutes_Gold"
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Solar", "int", "int", "factSmaPower15Minutes", "dw", "factSmaPower15Minutes", "Incremental",
    )
...
def main() -> int:
    ...
    try:
        cur.execute(
            "EXEC dw.usp_MergeFactSmaPower15Minutes @RunId = ?, @UltimoWatermark = ?",
