"""
Gold: [int].factGrowattEnergyAndPowerPv -> dw.factGrowattEnergyAndPowerPv
(INCREMENTAL).

No hay dimension de dispositivos Growatt en este dominio, asi que no hay
llave subrogada que resolver (a diferencia de los facts SMA/Huawei/
Soliscloud). El merge solo procesa filas cuyo FechaCargaInt en [int] sea
posterior al ultimo watermark de este proceso (independiente del watermark
de Silver). Todo ocurre dentro de JAREMAR -- este script solo orquesta el
ciclo de control (registro de proceso, log de corrida, watermark).

Uso:
    python db/etl/factGrowattEnergyAndPowerPv/load_gold_fact_growatt_energy_and_power_pv.py [--env-file .env]
"""
import argparse
import sys
...
PROCESO = "GrowattEnergyAndPowerPv_Gold"
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Solar", "int", "int", "factGrowattEnergyAndPowerPv", "dw", "factGrowattEnergyAndPowerPv", "Incremental",
    )
...
def main() -> int:
    ...
    try:
        cur.execute(
            "EXEC dw.usp_MergeFactGrowattEnergyAndPowerPv @RunId = ?, @UltimoWatermark = ?",
