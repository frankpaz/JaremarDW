"""
Gold: [int].dimGhiPlanDaily -> dw.dimGhiPlanDaily (SCD Tipo 1).

Todo ocurre dentro de JAREMAR -- el trabajo real lo hace
dw.usp_MergeDimGhiPlanDaily; este script solo orquesta el ciclo de control
(registro de proceso, log de corrida, watermark).

Uso:
    python db/etl/dimGhiPlanDaily/load_gold_dim_ghi_plan_daily.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Solar", "int", "int", "dimGhiPlanDaily", "dw", "dimGhiPlanDaily", "FULL",
    )
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC dw.usp_MergeDimGhiPlanDaily @RunId = ?", run_id)
