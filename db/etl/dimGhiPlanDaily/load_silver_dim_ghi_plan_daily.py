"""
Silver: stg.dimGhiPlanDaily -> [int].dimGhiPlanDaily (tipado, dedup por
Site+DayOfYear).

stg.dimGhiPlanDaily es un plan diario de irradiancia (P10/P50/P90) derivado
del Global Solar Atlas, curado manualmente -- no un extract_*.py de este
repo -- este pipeline arranca directo en Silver. Todo ocurre dentro de
JAREMAR -- el trabajo real lo hace [int].usp_MergeDimGhiPlanDaily; este
script solo orquesta el ciclo de control (registro de proceso, log de
corrida, watermark).

Uso:
    python db/etl/dimGhiPlanDaily/load_silver_dim_ghi_plan_daily.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Solar", "stg", "stg", "dimGhiPlanDaily", "int", "dimGhiPlanDaily", "FULL",
    )
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC [int].usp_MergeDimGhiPlanDaily @RunId = ?", run_id)
