"""
Silver: stg.dimCuenta -> [int].dimCuenta (tipado, dedup por SVSGVL).

Todo ocurre dentro de JAREMAR -- el trabajo real lo hace
[int].usp_MergeDimCuenta; este script solo orquesta el ciclo de control
(registro de proceso, log de corrida, watermark).

Uso:
    python db/etl/dimCuenta/load_silver_dim_cuenta.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Cuenta", "stg", "stg", "dimCuenta", "int", "dimCuenta", "FULL",
    )
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC [int].usp_MergeDimCuenta @RunId = ?", run_id)
