"""
Gold: [int].dimSmaPlants -> dw.dimSmaPlants (SCD Tipo 1).

Todo ocurre dentro de JAREMAR -- el trabajo real lo hace
dw.usp_MergeDimSmaPlants; este script solo orquesta el ciclo de control
(registro de proceso, log de corrida, watermark).

Uso:
    python db/etl/dimSmaPlants/load_gold_dim_sma_plants.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Solar", "int", "int", "dimSmaPlants", "dw", "dimSmaPlants", "FULL",
    )
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC dw.usp_MergeDimSmaPlants @RunId = ?", run_id)
