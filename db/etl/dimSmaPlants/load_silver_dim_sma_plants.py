"""
Silver: stg.dimSmaPlants -> [int].dimSmaPlants (tipado, dedup por PlantId).

stg.dimSmaPlants lo carga un proceso externo al dominio solar (API SMA), no
un extract_*.py de este repo -- este pipeline arranca directo en Silver.
Todo ocurre dentro de JAREMAR -- el trabajo real lo hace
[int].usp_MergeDimSmaPlants; este script solo orquesta el ciclo de control
(registro de proceso, log de corrida, watermark).

Uso:
    python db/etl/dimSmaPlants/load_silver_dim_sma_plants.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Solar", "stg", "stg", "dimSmaPlants", "int", "dimSmaPlants", "FULL",
    )
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC [int].usp_MergeDimSmaPlants @RunId = ?", run_id)
