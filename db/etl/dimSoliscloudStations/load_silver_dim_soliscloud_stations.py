"""
Silver: stg.dimSoliscloudStations -> [int].dimSoliscloudStations (tipado,
dedup por StationId).

stg.dimSoliscloudStations lo carga un proceso externo al dominio solar (API
Soliscloud), no un extract_*.py de este repo -- este pipeline arranca
directo en Silver. Todo ocurre dentro de JAREMAR -- el trabajo real lo hace
[int].usp_MergeDimSoliscloudStations; este script solo orquesta el ciclo de
control (registro de proceso, log de corrida, watermark).

Uso:
    python db/etl/dimSoliscloudStations/load_silver_dim_soliscloud_stations.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
PROCESO = "SoliscloudStations_Silver"
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Solar", "stg", "stg", "dimSoliscloudStations", "int", "dimSoliscloudStations", "FULL",
    )
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC [int].usp_MergeDimSoliscloudStations @RunId = ?", run_id)
