"""
Gold: [int].dimSoliscloudStations -> dw.dimSoliscloudStations (SCD Tipo 1).

Todo ocurre dentro de JAREMAR -- el trabajo real lo hace
dw.usp_MergeDimSoliscloudStations; este script solo orquesta el ciclo de
control (registro de proceso, log de corrida, watermark).

Uso:
    python db/etl/dimSoliscloudStations/load_gold_dim_soliscloud_stations.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
PROCESO = "SoliscloudStations_Gold"
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Solar", "int", "int", "dimSoliscloudStations", "dw", "dimSoliscloudStations", "FULL",
    )
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC dw.usp_MergeDimSoliscloudStations @RunId = ?", run_id)
