"""
Silver: stg.dimProveedor -> [int].dimProveedor (tipado, dedup por VENDOR).

Todo ocurre dentro de JAREMAR -- el trabajo real lo hace
[int].usp_MergeDimProveedor; este script solo orquesta el ciclo de control
(registro de proceso, log de corrida, watermark).

Uso:
    python db/etl/dimProveedor/load_silver_dim_proveedor.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Proveedor", "stg", "stg", "dimProveedor", "int", "dimProveedor", "FULL",
    )
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC [int].usp_MergeDimProveedor @RunId = ?", run_id)
