"""
Gold: [int].dimProveedor -> dw.dimProveedor (SCD Tipo 1, nombres de negocio).

Todo ocurre dentro de JAREMAR -- el trabajo real lo hace
dw.usp_MergeDimProveedor; este script solo orquesta el ciclo de control
(registro de proceso, log de corrida, watermark).

Uso:
    python db/etl/dimProveedor/load_gold_dim_proveedor.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Proveedor", "int", "int", "dimProveedor", "dw", "dimProveedor", "FULL",
    )
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC dw.usp_MergeDimProveedor @RunId = ?", run_id)
