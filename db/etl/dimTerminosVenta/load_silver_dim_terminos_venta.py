"""
Silver: stg.dimTerminosVenta -> [int].dimTerminosVenta (tipado, dedup por
TMCMPN+TMTERM).

Todo ocurre dentro de JAREMAR -- el trabajo real lo hace
[int].usp_MergeDimTerminosVenta; este script solo orquesta el ciclo de
control (registro de proceso, log de corrida, watermark).

Uso:
    python db/etl/dimTerminosVenta/load_silver_dim_terminos_venta.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC [int].usp_MergeDimTerminosVenta @RunId = ?", run_id)
