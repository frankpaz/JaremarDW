"""
Gold: [int].dimTerminosVenta -> dw.dimTerminosVenta (SCD Tipo 1, nombres de
negocio).

Todo ocurre dentro de JAREMAR -- el trabajo real lo hace
dw.usp_MergeDimTerminosVenta; este script solo orquesta el ciclo de control
(registro de proceso, log de corrida, watermark).

Uso:
    python db/etl/dimTerminosVenta/load_gold_dim_terminos_venta.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC dw.usp_MergeDimTerminosVenta @RunId = ?", run_id)
