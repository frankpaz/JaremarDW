"""
Extraccion Bronze: AS400/LX (PROLX835F.RTM, TMID='TM') -> stg.dimTerminosVenta
en JAREMAR.

RTM es el maestro de terminos de pago de ventas del ERP (Infor Distribution
SX.e, "A/R Terms Master File"). Es el catalogo que usa dw.factVentas
(SITERM) y dw.dimCliente (CTERM) -- distinto del catalogo de compras
(PROLX835F.AVT, ver dimTerminosCompra), que tiene su propia llave y
columnas: no son intercambiables, por eso son dos dimensiones separadas
(decision del usuario, 2026-09-22).

Llave de negocio TMCMPN+TMTERM (los terminos de venta se definen por
empresa, a diferencia de compras que son globales). 107 filas, unica
verificada.

Columnas elegidas tras analisis de poblacion real (2026-09-22, ver
db/discovery/discover_columnas.py --poblacion): la mayoria de los ~32
campos de configuracion avanzada (tramos de descuento escalonado, dias fijos
de mes, etc.) estan en 0% de uso -- solo se trajeron los realmente
poblados.

Carga FULL (truncate + insert).

Uso:
    python db/etl/dimTerminosVenta/extract_dim_terminos_venta.py [--env-file .env]
"""
import argparse
import datetime
import json
...
PROCESO = "TerminosVenta"
AS400_ESQUEMA_ORIGEN = "PROLX835F"
AS400_TABLA_ORIGEN = "RTM"
FILTRO_COLUMNA = "TMID"
FILTRO_VALOR = "TM"
STG_ESQUEMA = "stg"
STG_TABLA = "dimTerminosVenta"

COLUMNAS_DESEADAS = ["TMCMPN", "TMTERM", "TMDESC", "TMDUE", "TMTAXD", "TMBBTA", "TMCWO"]
...
def cargar_stg(jrm_conn, jrm_cur, columnas: list, filas: list, run_id: int) -> tuple:
    ...
    llave_idx = nombres_cols.index("TMTERM") if "TMTERM" in nombres_cols else None
