"""
Gold: [int].dimGsaMonthlyReference -> dw.dimGsaMonthlyReference (SCD Tipo 1).

Todo ocurre dentro de JAREMAR -- el trabajo real lo hace
dw.usp_MergeDimGsaMonthlyReference; este script solo orquesta el ciclo de
control (registro de proceso, log de corrida, watermark).

Uso:
    python db/etl/dimGsaMonthlyReference/load_gold_dim_gsa_monthly_reference.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
PROCESO = "GsaMonthlyReference_Gold"
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Solar", "int", "int", "dimGsaMonthlyReference", "dw", "dimGsaMonthlyReference", "FULL",
    )
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC dw.usp_MergeDimGsaMonthlyReference @RunId = ?", run_id)
