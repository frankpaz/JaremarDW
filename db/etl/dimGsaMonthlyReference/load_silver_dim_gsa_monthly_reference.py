"""
Silver: stg.dimGsaMonthlyReference -> [int].dimGsaMonthlyReference (tipado,
dedup por Site+MonthOfYear).

stg.dimGsaMonthlyReference es una referencia curada manualmente (irradiancia
mensual del Global Solar Atlas por sitio), no un extract_*.py de este repo
-- este pipeline arranca directo en Silver. Todo ocurre dentro de JAREMAR --
el trabajo real lo hace [int].usp_MergeDimGsaMonthlyReference; este script
solo orquesta el ciclo de control (registro de proceso, log de corrida,
watermark).

Uso:
    python db/etl/dimGsaMonthlyReference/load_silver_dim_gsa_monthly_reference.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
PROCESO = "GsaMonthlyReference_Silver"
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Solar", "stg", "stg", "dimGsaMonthlyReference", "int", "dimGsaMonthlyReference", "FULL",
    )
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC [int].usp_MergeDimGsaMonthlyReference @RunId = ?", run_id)
