"""
Silver: stg.dimSubCuenta -> [int].dimSubCuenta (tipado, dedup por SVSGVL).

Todo ocurre dentro de JAREMAR -- el trabajo real lo hace
[int].usp_MergeDimSubCuenta; este script solo orquesta el ciclo de control
(registro de proceso, log de corrida, watermark).

Uso:
    python db/etl/dimSubCuenta/load_silver_dim_subcuenta.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "SubCuenta", "stg", "stg", "dimSubCuenta", "int", "dimSubCuenta", "FULL",
    )
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC [int].usp_MergeDimSubCuenta @RunId = ?", run_id)
