"""
Gold: [int].dimSubCuenta -> dw.dimSubCuenta (SCD Tipo 1, nombres de negocio).

Todo ocurre dentro de JAREMAR -- el trabajo real lo hace
dw.usp_MergeDimSubCuenta; este script solo orquesta el ciclo de control
(registro de proceso, log de corrida, watermark).

Uso:
    python db/etl/dimSubCuenta/load_gold_dim_subcuenta.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "SubCuenta", "int", "int", "dimSubCuenta", "dw", "dimSubCuenta", "FULL",
    )
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC dw.usp_MergeDimSubCuenta @RunId = ?", run_id)
