"""
Silver: stg.factEnvios -> [int].factEnvios (tipado, dedup por ENCENV,
patron FULL + SCD Tipo 1).

Todo ocurre dentro de JAREMAR -- el trabajo real lo hace
[int].usp_MergeFactEnvios; este script solo orquesta el ciclo de control
(registro de proceso, log de corrida, watermark).

Uso:
    python db/etl/factEnvios/load_silver_fact_envios.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Envios", "stg", "stg", "factEnvios", "int", "factEnvios", "FULL",
    )
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC [int].usp_MergeFactEnvios @RunId = ?", run_id)
