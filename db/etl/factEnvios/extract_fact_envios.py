...
origen no tiene ninguna columna confiable de fecha/hora de ultima
modificacion (ENCFEU/ENCTIU existen pero nunca se pueblan -- confirmado
consultando el origen: 0 filas con valor distinto de 0 en 211292 filas), por
lo que no es posible extraer solo lo "nuevo" desde el AS400. Silver resuelve
el estado actual via dedup + HashDiff (patron FULL + SCD Tipo 1, igual que
factMeteoDaily), no via watermark real.
...
Uso:
    python db/etl/factEnvios/extract_fact_envios.py [--env-file .env]
"""
import argparse
import datetime
import json
