"""
Gold: [int].factSoliscloudEnergyAndPowerPv -> dw.factSoliscloudEnergyAndPowerPv
(INCREMENTAL).

Requiere que dw.dimSoliscloudDevices ya este cargado
(dw.usp_MergeFactSoliscloudEnergyAndPowerPv resuelve SoliscloudDeviceKey por
DeviceId contra esa tabla via LEFT JOIN). El merge solo procesa filas cuyo
FechaCargaInt en [int] sea posterior al ultimo watermark de este proceso
(independiente del watermark de Silver). Todo ocurre dentro de JAREMAR --
este script solo orquesta el ciclo de control (registro de proceso, log de
corrida, watermark).

Uso:
    python db/etl/factSoliscloudEnergyAndPowerPv/load_gold_fact_soliscloud_energy_and_power_pv.py [--env-file .env]
"""
import argparse
import sys
...
PROCESO = "SoliscloudEnergyAndPowerPv_Gold"
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Solar", "int", "int", "factSoliscloudEnergyAndPowerPv", "dw", "factSoliscloudEnergyAndPowerPv", "Incremental",
    )
...
def main() -> int:
    ...
    try:
        cur.execute(
            "EXEC dw.usp_MergeFactSoliscloudEnergyAndPowerPv @RunId = ?, @UltimoWatermark = ?",
