"""
Silver: stg.factSoliscloudEnergyAndPowerPv -> [int].factSoliscloudEnergyAndPowerPv
(tipado, dedup por Id, INCREMENTAL).

stg.factSoliscloudEnergyAndPowerPv lo carga un proceso externo al dominio
solar (API Soliscloud), no un extract_*.py de este repo -- este pipeline
...
LastModificationTime posteriores al ultimo watermark del proceso. La
telemetria cruda del inversor llega como JSON (KpiData, ~128 claves) y se
parsea a columnas tipadas en
[int].usp_MergeFactSoliscloudEnergyAndPowerPv.

Todo ocurre dentro de JAREMAR -- este script solo orquesta el ciclo de
control (registro de proceso, log de corrida, watermark).

Uso:
    python db/etl/factSoliscloudEnergyAndPowerPv/load_silver_fact_soliscloud_energy_and_power_pv.py [--env-file .env]
"""
import argparse
import sys
...
PROCESO = "SoliscloudEnergyAndPowerPv_Silver"
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Solar", "stg", "stg", "factSoliscloudEnergyAndPowerPv", "int", "factSoliscloudEnergyAndPowerPv", "Incremental",
    )
...
def main() -> int:
    ...
    try:
        cur.execute(
            "EXEC [int].usp_MergeFactSoliscloudEnergyAndPowerPv @RunId = ?, @UltimoWatermark = ?",
