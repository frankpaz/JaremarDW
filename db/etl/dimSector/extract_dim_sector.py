"""
Extraccion Bronze: AS400/LX (PROLX835F.GSV, SVSGMN='SECTOR', SVSGVL entre
'000' y '999') -> stg.dimSector en JAREMAR.

Carga FULL (truncate + insert), sin transformar los datos del origen. El
filtro SVSGVL BETWEEN '000' AND '999' excluye codigos de proyecto (ej. 'A01',
'PYSAT-...') que comparten el mismo SVSGMN='SECTOR' pero no son sectores
reales -- viene asi en el query real del equipo de datos. La estructura de
stg.dimSector se auto-provisiona a partir de la metadata real de las
columnas en el AS400.

Uso:
    python db/etl/dimSector/extract_dim_sector.py [--env-file .env]
"""
import argparse
import datetime
import json
...
FILTRO_VALOR = "SECTOR"
FILTRO_RANGO_COLUMNA = "SVSGVL"
FILTRO_RANGO_DESDE = "000"
FILTRO_RANGO_HASTA = "999"
STG_ESQUEMA = "stg"
STG_TABLA = "dimSector"
...
def extraer_filas(as400_cur: pyodbc.Cursor, columnas: list) -> list:
    ...
    as400_cur.execute(
        ...
        f'WHERE "{FILTRO_COLUMNA}" = ? AND "{FILTRO_RANGO_COLUMNA}" BETWEEN ? AND ?',
        FILTRO_VALOR, FILTRO_RANGO_DESDE, FILTRO_RANGO_HASTA,
    )
