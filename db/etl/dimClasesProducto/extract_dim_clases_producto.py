"""
Extraccion Bronze: AS400/LX (PROLX835F.IIC) -> stg.dimClasesProducto en JAREMAR.

Carga FULL (truncate + insert), sin transformar los datos del origen. Trae
las columnas confirmadas por el equipo de datos (query real: "select iclas,
icdes from prolx835f.iic"). Tabla de referencia -- sin filtro de tipo de
registro (a diferencia de IIM). La estructura de stg.dimClasesProducto se
...
AS400_TABLA_ORIGEN = "IIC"
STG_ESQUEMA = "stg"
STG_TABLA = "dimClasesProducto"

# Columnas confirmadas por el equipo de datos (query real que ya usan)
COLUMNAS_DESEADAS = ["ICLAS", "ICDES"]
...
def cargar_stg(jrm_conn, jrm_cur, columnas: list, filas: list, run_id: int) -> tuple:
    ...
    llave_idx = nombres_cols.index("ICLAS") if "ICLAS" in nombres_cols else None
