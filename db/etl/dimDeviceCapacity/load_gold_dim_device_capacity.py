"""
Gold: [int].dimDeviceCapacity -> dw.dimDeviceCapacity (SCD Tipo 1).

Todo ocurre dentro de JAREMAR -- el trabajo real lo hace
dw.usp_MergeDimDeviceCapacity; este script solo orquesta el ciclo de
control (registro de proceso, log de corrida, watermark).

Uso:
    python db/etl/dimDeviceCapacity/load_gold_dim_device_capacity.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Solar", "int", "int", "dimDeviceCapacity", "dw", "dimDeviceCapacity", "FULL",
    )
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC dw.usp_MergeDimDeviceCapacity @RunId = ?", run_id)
