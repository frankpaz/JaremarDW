"""
Silver: stg.dimDeviceCapacity -> [int].dimDeviceCapacity (tipado, dedup por
InverterId).

stg.dimDeviceCapacity es un catalogo curado manualmente (capacidad nameplate
por inversor, cruzando los 4 vendors del dominio solar), no un extract_*.py
de este repo -- este pipeline arranca directo en Silver. Todo ocurre dentro
de JAREMAR -- el trabajo real lo hace [int].usp_MergeDimDeviceCapacity; este
script solo orquesta el ciclo de control (registro de proceso, log de
corrida, watermark).

Uso:
    python db/etl/dimDeviceCapacity/load_silver_dim_device_capacity.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Solar", "stg", "stg", "dimDeviceCapacity", "int", "dimDeviceCapacity", "FULL",
    )
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC [int].usp_MergeDimDeviceCapacity @RunId = ?", run_id)
