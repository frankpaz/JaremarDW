"""
Gold: [int].dimTerminosCompra -> dw.dimTerminosCompra (SCD Tipo 1, nombres
de negocio).

Todo ocurre dentro de JAREMAR -- el trabajo real lo hace
dw.usp_MergeDimTerminosCompra; este script solo orquesta el ciclo de
control (registro de proceso, log de corrida, watermark).

Uso:
    python db/etl/dimTerminosCompra/load_gold_dim_terminos_compra.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC dw.usp_MergeDimTerminosCompra @RunId = ?", run_id)
