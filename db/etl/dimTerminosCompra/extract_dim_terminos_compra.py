"""
Extraccion Bronze: AS400/LX (PROLX835F.AVT, VTMID='VT') -> stg.dimTerminosCompra
en JAREMAR.

AVT es el maestro de terminos de pago de compras del ERP (Infor Distribution
SX.e, "Vendor Terms"). Es el catalogo que usa dw.factCompras (APTERM) y
dw.dimProveedor (VTERMS) -- distinto del catalogo de ventas
(PROLX835F.RTM, ver dimTerminosVenta): AVT es global (llave solo VTERM, sin
distincion por empresa), RTM es por empresa. No son intercambiables, por
eso son dos dimensiones separadas (decision del usuario, 2026-09-22).

Llave de negocio VTERM. 12 filas reales (VTMID='VT'; hay 1 fila VTMID='VZ'
que se excluye, mismo patron de discriminador que AVM/RCM aunque aqui es
casi anecdotico por el volumen).

Columnas elegidas tras analisis de poblacion real (2026-09-22, ver
db/discovery/discover_columnas.py --poblacion): la mayoria de los ~17
campos de configuracion avanzada estan en 0% de uso -- solo se trajeron los
realmente poblados.

Carga FULL (truncate + insert).

Uso:
    python db/etl/dimTerminosCompra/extract_dim_terminos_compra.py [--env-file .env]
"""
import argparse
import datetime
import json
...
PROCESO = "TerminosCompra"
AS400_ESQUEMA_ORIGEN = "PROLX835F"
AS400_TABLA_ORIGEN = "AVT"
FILTRO_COLUMNA = "VTMID"
FILTRO_VALOR = "VT"
STG_ESQUEMA = "stg"
STG_TABLA = "dimTerminosCompra"

COLUMNAS_DESEADAS = ["VTERM", "VTMDSC", "VTMDDY", "VTTAXD"]
...
def cargar_stg(jrm_conn, jrm_cur, columnas: list, filas: list, run_id: int) -> tuple:
    ...
    llave_idx = nombres_cols.index("VTERM") if "VTERM" in nombres_cols else None
