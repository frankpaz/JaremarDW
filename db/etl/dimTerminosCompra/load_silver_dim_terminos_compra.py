"""
Silver: stg.dimTerminosCompra -> [int].dimTerminosCompra (tipado, dedup por
VTERM).

Todo ocurre dentro de JAREMAR -- el trabajo real lo hace
[int].usp_MergeDimTerminosCompra; este script solo orquesta el ciclo de
control (registro de proceso, log de corrida, watermark).

Uso:
    python db/etl/dimTerminosCompra/load_silver_dim_terminos_compra.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC [int].usp_MergeDimTerminosCompra @RunId = ?", run_id)
