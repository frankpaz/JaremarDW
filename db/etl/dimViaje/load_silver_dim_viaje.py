"""
Silver: stg.dimViaje -> [int].dimViaje (tipado, dedup por VCODPA).

Todo ocurre dentro de JAREMAR -- el trabajo real lo hace
[int].usp_MergeDimViaje; este script solo orquesta el ciclo de control
(registro de proceso, log de corrida, watermark).

Uso:
    python db/etl/dimViaje/load_silver_dim_viaje.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Viaje", "stg", "stg", "dimViaje", "int", "dimViaje", "FULL",
    )
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC [int].usp_MergeDimViaje @RunId = ?", run_id)
