...
Uso:
    python db/etl/dimViaje/extract_dim_viaje.py [--env-file .env]
"""
import argparse
import datetime
import json
...
# Las 11 columnas del origen (discovery 2026-09-26, QSYS2.SYSCOLUMNS)
COLUMNAS_DESEADAS = [
    "VCODPA", "VDESC", "VPAGAM", "VPAGAA",
    "VUSUAG", "VFECHG", "VHORAG", "VUSUAM", "VFECHM", "VHORAM", "VSTS",
]
...
def cargar_stg(jrm_conn, jrm_cur, columnas: list, filas: list, run_id: int) -> tuple:
    ...
    llave_idx = nombres_cols.index("VCODPA") if "VCODPA" in nombres_cols else None
