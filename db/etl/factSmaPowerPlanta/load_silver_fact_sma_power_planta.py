"""
Silver: stg.factSmaPowerPlanta -> [int].factSmaPowerPlanta (tipado, dedup por
Id, INCREMENTAL).

stg.factSmaPowerPlanta lo carga un proceso externo al dominio solar (API
SMA), no un extract_*.py de este repo -- este pipeline arranca directo en
Silver. A diferencia de las dimensiones (carga FULL), este es un fact table
de serie de tiempo: el merge solo procesa filas con CreationTime/
LastModificationTime posteriores al ultimo watermark del proceso, leido de
dbo.usp_Etl_WatermarkObtener y actualizado al final con el maximo
CreationTime/LastModificationTime visto en el batch (no con la hora actual).

Todo ocurre dentro de JAREMAR -- el trabajo real lo hace
[int].usp_MergeFactSmaPowerPlanta; este script solo orquesta el ciclo de
control (registro de proceso, log de corrida, watermark).

Uso:
    python db/etl/factSmaPowerPlanta/load_silver_fact_sma_power_planta.py [--env-file .env]
"""
import argparse
import sys
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Solar", "stg", "stg", "factSmaPowerPlanta", "int", "factSmaPowerPlanta", "Incremental",
    )
...
def main() -> int:
    ...
    try:
        cur.execute(
            "EXEC [int].usp_MergeFactSmaPowerPlanta @RunId = ?, @UltimoWatermark = ?",
