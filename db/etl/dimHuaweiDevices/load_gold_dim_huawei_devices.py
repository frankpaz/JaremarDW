"""
Gold: [int].dimHuaweiDevices -> dw.dimHuaweiDevices (SCD Tipo 1).

Requiere que dw.dimHuaweiStations ya este cargado (dw.usp_MergeDimHuaweiDevices
resuelve HuaweiStationKey por StationCode contra esa tabla). Todo ocurre
dentro de JAREMAR -- este script solo orquesta el ciclo de control (registro
de proceso, log de corrida, watermark).

Uso:
    python db/etl/dimHuaweiDevices/load_gold_dim_huawei_devices.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Solar", "int", "int", "dimHuaweiDevices", "dw", "dimHuaweiDevices", "FULL",
    )
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC dw.usp_MergeDimHuaweiDevices @RunId = ?", run_id)
