"""
Silver: stg.dimHuaweiDevices -> [int].dimHuaweiDevices (tipado, dedup por DeviceId).

stg.dimHuaweiDevices lo carga un proceso externo al dominio solar (API
...
lo hace [int].usp_MergeDimHuaweiDevices; este script solo orquesta el ciclo
de control (registro de proceso, log de corrida, watermark).

Uso:
    python db/etl/dimHuaweiDevices/load_silver_dim_huawei_devices.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Solar", "stg", "stg", "dimHuaweiDevices", "int", "dimHuaweiDevices", "FULL",
    )
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC [int].usp_MergeDimHuaweiDevices @RunId = ?", run_id)
