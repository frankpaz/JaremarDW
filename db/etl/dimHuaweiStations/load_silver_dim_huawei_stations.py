"""
Silver: stg.dimHuaweiStations -> [int].dimHuaweiStations (tipado, dedup por StationCode).

stg.dimHuaweiStations lo carga un proceso externo al dominio solar (API
Huawei FusionSolar), no un extract_*.py de este repo -- este pipeline
arranca directo en Silver. Todo ocurre dentro de JAREMAR -- el trabajo real
lo hace [int].usp_MergeDimHuaweiStations; este script solo orquesta el
ciclo de control (registro de proceso, log de corrida, watermark).

Uso:
    python db/etl/dimHuaweiStations/load_silver_dim_huawei_stations.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Solar", "stg", "stg", "dimHuaweiStations", "int", "dimHuaweiStations", "FULL",
    )
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC [int].usp_MergeDimHuaweiStations @RunId = ?", run_id)
