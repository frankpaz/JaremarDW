"""
Gold: [int].dimHuaweiStations -> dw.dimHuaweiStations (SCD Tipo 1).

Todo ocurre dentro de JAREMAR -- el trabajo real lo hace
dw.usp_MergeDimHuaweiStations; este script solo orquesta el ciclo de
control (registro de proceso, log de corrida, watermark).

Uso:
    python db/etl/dimHuaweiStations/load_gold_dim_huawei_stations.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Solar", "int", "int", "dimHuaweiStations", "dw", "dimHuaweiStations", "FULL",
    )
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC dw.usp_MergeDimHuaweiStations @RunId = ?", run_id)
