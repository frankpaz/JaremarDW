"""
Gold: [int].dimSmaDevices -> dw.dimSmaDevices (SCD Tipo 1).

Requiere que dw.dimSmaPlants ya este cargado (dw.usp_MergeDimSmaDevices
resuelve SmaPlantaKey por PlantId contra esa tabla). Todo ocurre dentro de
JAREMAR -- este script solo orquesta el ciclo de control (registro de
proceso, log de corrida, watermark).

Uso:
    python db/etl/dimSmaDevices/load_gold_dim_sma_devices.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Solar", "int", "int", "dimSmaDevices", "dw", "dimSmaDevices", "FULL",
    )
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC dw.usp_MergeDimSmaDevices @RunId = ?", run_id)
