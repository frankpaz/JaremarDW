"""
Silver: stg.dimSmaDevices -> [int].dimSmaDevices (tipado, dedup por DeviceId).

stg.dimSmaDevices lo carga un proceso externo al dominio solar (API SMA), no
un extract_*.py de este repo -- este pipeline arranca directo en Silver.
Todo ocurre dentro de JAREMAR -- el trabajo real lo hace
[int].usp_MergeDimSmaDevices; este script solo orquesta el ciclo de control
(registro de proceso, log de corrida, watermark).

Uso:
    python db/etl/dimSmaDevices/load_silver_dim_sma_devices.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Solar", "stg", "stg", "dimSmaDevices", "int", "dimSmaDevices", "FULL",
    )
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC [int].usp_MergeDimSmaDevices @RunId = ?", run_id)
