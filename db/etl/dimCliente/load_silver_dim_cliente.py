"""
Silver: stg.dimCliente -> [int].dimCliente (tipado, dedup por CCUST).

Todo ocurre dentro de JAREMAR -- el trabajo real lo hace
[int].usp_MergeDimCliente; este script solo orquesta el ciclo de control
(registro de proceso, log de corrida, watermark).

Uso:
    python db/etl/dimCliente/load_silver_dim_cliente.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Cliente", "stg", "stg", "dimCliente", "int", "dimCliente", "FULL",
    )
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC [int].usp_MergeDimCliente @RunId = ?", run_id)
