"""
Gold: [int].dimProducto -> dw.dimProducto (SCD Tipo 1, nombres de negocio).

Todo ocurre dentro de JAREMAR -- el trabajo real lo hace
dw.usp_MergeDimProducto; este script solo orquesta el ciclo de control
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Producto", "int", "int", "dimProducto", "dw", "dimProducto", "FULL",
    )
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC dw.usp_MergeDimProducto @RunId = ?", run_id)
