"""
Extraccion Bronze: AS400/LX (PROLX835F.LCM, "Freight Carrier Codes Master")
-> stg.dimVehiculo en JAREMAR.

Aunque el origen la describe como maestro de transportistas, cada registro es
un vehiculo (placa, capacidad, tipo, empresa propietaria, motorista); CMCARR
es el mismo codigo que ENCCAM (camion) de los envios. Se traen las dos
poblaciones de CMID ('CM' activos y 'CZ', 22 vehiculos que parecen dados de
baja pero siguen apareciendo en envios): CMCARR es unico entre ambas.

Carga FULL (truncate + insert), sin transformar los datos del origen. Se traen
solo las columnas con uso real (ADR3/ADR4, PERP, DPCT, COMC, FAX, LANG, APTL,
CALN y RGCD estan vacias o casi vacias; CMAMNF es constante). La estructura de
stg.dimVehiculo se auto-provisiona a partir de la metadata real de las
columnas en el AS400.

Uso:
    python db/etl/dimVehiculo/extract_dim_vehiculo.py [--env-file .env]
"""
import argparse
import datetime
import json
...
AS400_TABLA_ORIGEN = "LCM"
STG_ESQUEMA = "stg"
STG_TABLA = "dimVehiculo"

# Columnas con uso real (discovery + analisis de poblacion 2026-09-26)
COLUMNAS_DESEADAS = [
    "CMID", "CMCARR", "CMCDES", "CMXDES", "CMADR1", "CMADR2", "CMSTCD", "CMPSCD",
    "CMCNTY", "CMSHPC", "CMPERR", "CMVEND", "CMINVF", "CMLUSR", "CMLDTE", "CMLTME",
    "CMADR5", "CMADR6", "CMATTN", "CMDATN", "CMPHON", "CMFRCC",
]
...
def cargar_stg(jrm_conn, jrm_cur, columnas: list, filas: list, run_id: int) -> tuple:
    ...
    llave_idx = nombres_cols.index("CMCARR") if "CMCARR" in nombres_cols else None
