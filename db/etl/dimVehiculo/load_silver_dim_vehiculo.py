"""
Silver: stg.dimVehiculo -> [int].dimVehiculo (tipado, dedup por CMCARR).

Todo ocurre dentro de JAREMAR -- el trabajo real lo hace
[int].usp_MergeDimVehiculo; este script solo orquesta el ciclo de control
(registro de proceso, log de corrida, watermark).

Uso:
    python db/etl/dimVehiculo/load_silver_dim_vehiculo.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Vehiculo", "stg", "stg", "dimVehiculo", "int", "dimVehiculo", "FULL",
    )
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC [int].usp_MergeDimVehiculo @RunId = ?", run_id)
