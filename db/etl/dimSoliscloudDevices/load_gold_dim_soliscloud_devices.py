"""
Gold: [int].dimSoliscloudDevices -> dw.dimSoliscloudDevices (SCD Tipo 1).

Requiere que dw.dimSoliscloudStations ya este cargado
(dw.usp_MergeDimSoliscloudDevices resuelve SoliscloudStationKey por
StationId contra esa tabla). Todo ocurre dentro de JAREMAR -- este script
solo orquesta el ciclo de control (registro de proceso, log de corrida,
watermark).

Uso:
    python db/etl/dimSoliscloudDevices/load_gold_dim_soliscloud_devices.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
PROCESO = "SoliscloudDevices_Gold"
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Solar", "int", "int", "dimSoliscloudDevices", "dw", "dimSoliscloudDevices", "FULL",
    )
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC dw.usp_MergeDimSoliscloudDevices @RunId = ?", run_id)
