"""
Silver: stg.dimSoliscloudDevices -> [int].dimSoliscloudDevices (tipado, dedup
por DeviceId).

stg.dimSoliscloudDevices lo carga un proceso externo al dominio solar (API
...
[int].usp_MergeDimSoliscloudDevices; este script solo orquesta el ciclo de
control (registro de proceso, log de corrida, watermark).

Uso:
    python db/etl/dimSoliscloudDevices/load_silver_dim_soliscloud_devices.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
PROCESO = "SoliscloudDevices_Silver"
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Solar", "stg", "stg", "dimSoliscloudDevices", "int", "dimSoliscloudDevices", "FULL",
    )
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC [int].usp_MergeDimSoliscloudDevices @RunId = ?", run_id)
