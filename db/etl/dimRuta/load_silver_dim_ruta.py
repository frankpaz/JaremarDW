"""
Silver: stg.dimRuta -> [int].dimRuta (tipado, dedup por CCCODE).

Todo ocurre dentro de JAREMAR -- el trabajo real lo hace
[int].usp_MergeDimRuta; este script solo orquesta el ciclo de control
(registro de proceso, log de corrida, watermark).

Uso:
    python db/etl/dimRuta/load_silver_dim_ruta.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Ruta", "stg", "stg", "dimRuta", "int", "dimRuta", "FULL",
    )
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC [int].usp_MergeDimRuta @RunId = ?", run_id)
