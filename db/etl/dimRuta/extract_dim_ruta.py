"""
Extraccion Bronze: AS400/LX (PROLX835F.ZCC, CCTABL='ROUTE') -> stg.dimRuta en JAREMAR.

ZCC es una tabla generica de codigos; CCTABL='ROUTE' es el catalogo de rutas
(514 filas, llave CCCODE unica). Carga FULL (truncate + insert), sin transformar
los datos del origen. Se traen solo las columnas con uso real (CCLANG, CCALTC,
CCSDSC, CCNOT1/2, CCUDC1-3, CCRESV y los indicadores de revision estan vacios o
casi vacios). Los textos del origen son GRAPHIC: se mapean a NVARCHAR con su
largo real. La estructura de stg.dimRuta se auto-provisiona a partir de la
metadata real de las columnas en el AS400.

Uso:
    python db/etl/dimRuta/extract_dim_ruta.py [--env-file .env]
"""
import argparse
import datetime
import json
...
AS400_TABLA_ORIGEN = "ZCC"
FILTRO_COLUMNA = "CCTABL"
FILTRO_VALOR = "ROUTE"
STG_ESQUEMA = "stg"
STG_TABLA = "dimRuta"

# Columnas confirmadas por el equipo de datos (query real que ya usan)
COLUMNAS_DESEADAS = ["CCCODE", "CCDESC", "CCENDT", "CCENTM", "CCENUS", "CCMNDT", "CCMNTM", "CCMNUS"]
...
def cargar_stg(jrm_conn, jrm_cur, columnas: list, filas: list, run_id: int) -> tuple:
    ...
    llave_idx = nombres_cols.index("CCCODE") if "CCCODE" in nombres_cols else None
