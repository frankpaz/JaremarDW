"""
Silver: stg.dimPais -> [int].dimPais (tipado, dedup por CNCNTY).

Todo ocurre dentro de JAREMAR -- el trabajo real lo hace
[int].usp_MergeDimPais; este script solo orquesta el ciclo de control
(registro de proceso, log de corrida, watermark).

Uso:
    python db/etl/dimPais/load_silver_dim_pais.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Geografia", "stg", "stg", "dimPais", "int", "dimPais", "FULL",
    )
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC [int].usp_MergeDimPais @RunId = ?", run_id)
