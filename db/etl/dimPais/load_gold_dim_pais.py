"""
Gold: [int].dimPais -> dw.dimPais (SCD Tipo 1, nombres de negocio,
enriquecido con ref.PaisesIso3166).

Todo ocurre dentro de JAREMAR -- el trabajo real lo hace
dw.usp_MergeDimPais; este script solo orquesta el ciclo de control
(registro de proceso, log de corrida, watermark).

Uso:
    python db/etl/dimPais/load_gold_dim_pais.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Geografia", "int", "int", "dimPais", "dw", "dimPais", "FULL",
    )
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC dw.usp_MergeDimPais @RunId = ?", run_id)
