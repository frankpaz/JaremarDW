"""
Extraccion Bronze: AS400/LX (PROLX835F.LCN, CNID='CN') -> stg.dimPais en
JAREMAR.

LCN es el maestro de paises del ERP (Infor Distribution SX.e, "Country
Master"), con codigo estilo ISO 3166-1 alpha-3 (CNCNTY, ej. HND/GTM/SLV) y
descripcion en espanol. 244 filas, CNCNTY unico verificado.

Columnas elegidas tras analisis de poblacion real (2026-09-22, ver
db/discovery/discover_columnas.py --poblacion): se descartaron ~20 columnas
de configuracion de formato de direccion (secuencia de campos, longitud de
codigo postal, etc.), todas en 0% de uso.

Carga FULL (truncate + insert). En Gold, dw.dimPais se enriquece con una
tabla de referencia estatica (ref.PaisesIso3166, sembrada por migracion, no
viene del AS400) para agregar codigo alpha-2 y numerico ISO -- LCN solo
trae alpha-3.

Uso:
    python db/etl/dimPais/extract_dim_pais.py [--env-file .env]
"""
import argparse
import datetime
import json
...
COLUMNAS_DESEADAS = [
    "CNCNTY", "CNLDSC", "CNSDSC", "CNLANG",
    "CNLUSR", "CNLDTE", "CNLTME",
    "CNCUSR", "CNCDTE", "CNCTME",
]
...
def cargar_stg(jrm_conn, jrm_cur, columnas: list, filas: list, run_id: int) -> tuple:
    ...
    llave_idx = nombres_cols.index("CNCNTY") if "CNCNTY" in nombres_cols else None
