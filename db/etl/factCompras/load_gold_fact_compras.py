"""
Gold: [int].factCompras -> dw.factCompras (nombres de negocio, FKs a
dimEmpresas/dimProveedor). Incremental puro, sin SCD/EsVigente.

Todo ocurre dentro de JAREMAR -- el trabajo real lo hace
dw.usp_MergeFactCompras; este script solo orquesta el ciclo de control
(registro de proceso, log de corrida, watermark).

Uso:
    python db/etl/factCompras/load_gold_fact_compras.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Compras", "int", "int", "factCompras", "dw", "factCompras", "Incremental",
    )
...
def main() -> int:
    ...
    try:
        cur.execute(
            "EXEC dw.usp_MergeFactCompras @RunId = ?, @UltimoWatermark = ?, @Reconciliar = ?",
