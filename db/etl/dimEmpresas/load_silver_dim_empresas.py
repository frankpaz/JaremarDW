"""
Silver: stg.dimEmpresas -> [int].dimEmpresas (tipado, dedup por SVSGVL).

Todo ocurre dentro de JAREMAR -- el trabajo real lo hace
[int].usp_MergeDimEmpresas; este script solo orquesta el ciclo de control
(registro de proceso, log de corrida, watermark).

Uso:
    python db/etl/dimEmpresas/load_silver_dim_empresas.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Empresas", "stg", "stg", "dimEmpresas", "int", "dimEmpresas", "FULL",
    )
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC [int].usp_MergeDimEmpresas @RunId = ?", run_id)
