"""
Extraccion Bronze: AS400/LX (PROLX835F.RCO) -> stg.dimEmpresaMoneda en JAREMAR.

Compania por moneda (relacion 1:1 -- cada compania tiene exactamente una
moneda funcional en el origen). Se aterriza como su propia tabla Bronze,
fiel al origen; el enriquecimiento real hacia dimEmpresas ocurre en Silver
(ver int.usp_MergeDimEmpresas), que hace JOIN contra esta tabla.

Carga FULL (truncate + insert), sin transformar los datos del origen. Este
extract debe correr ANTES de load_silver_dim_empresas.py para que el JOIN
tenga datos frescos.

Uso:
    python db/etl/dimEmpresas/extract_dim_empresa_moneda.py [--env-file .env]
"""
import argparse
import datetime
import json
...
PROCESO = "EmpresaMoneda"
AS400_ESQUEMA_ORIGEN = "PROLX835F"
AS400_TABLA_ORIGEN = "RCO"
STG_ESQUEMA = "stg"
STG_TABLA = "dimEmpresaMoneda"

# Columnas confirmadas por el equipo de datos (query real que ya usan)
COLUMNAS_DESEADAS = ["CMPNY", "CCURCY"]
...
def cargar_stg(jrm_conn, jrm_cur, columnas: list, filas: list, run_id: int) -> tuple:
    ...
    llave_idx = nombres_cols.index("CMPNY") if "CMPNY" in nombres_cols else None
