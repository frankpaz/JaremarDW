"""
Silver: stg.dimProducto -> [int].dimProducto (limpieza, tipado, dedup por IPROD).

Todo ocurre dentro de JAREMAR (no hay origen externo) -- el trabajo real lo
hace [int].usp_MergeDimProducto; este script solo orquesta el ciclo de
control (registro de proceso, log de corrida, watermark).

Uso:
    python db/etl/dimProducto/load_silver_dim_producto.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "Producto", "stg", "stg", "dimProducto", "int", "dimProducto", "FULL",
    )
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC [int].usp_MergeDimProducto @RunId = ?", run_id)
