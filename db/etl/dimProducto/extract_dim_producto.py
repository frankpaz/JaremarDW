"""
Extraccion Bronze: AS400/LX (PROLX835F.IIM, IID='IM') -> stg.dimProducto en JAREMAR.

Carga FULL (truncate + insert), sin transformar los datos del origen. Trae
solo el subconjunto de columnas dado por el equipo de datos (COLUMNAS_DESEADAS)
-- PROLX835F.IIM tiene 340+ columnas en total, pero solo estas son las que se
necesitan. La estructura de stg.dimProducto se auto-provisiona a partir de la
metadata real de esas columnas en el AS400 (evita adivinar precision/escala
...
AS400_TABLA_ORIGEN = "IIM"
FILTRO_TIPO_REGISTRO = "IM"  # IID = 'IM' -> registros de tipo item master
STG_ESQUEMA = "stg"
STG_TABLA = "dimProducto"

# Columnas confirmadas por el equipo de datos (query real que ya usan)
COLUMNAS_DESEADAS = [
    "IID", "IPROD", "IDESC", "ICLAS", "IUMS", "IUMP", "ILDTE", "IMMNDT", "IMMNTM",
]
...
def asegurar_tabla_stg(jrm_cur: pyodbc.Cursor, columnas: list) -> str:
    ...
    # Si la tabla existe pero con una estructura distinta a la esperada
    # (ej. quedo de un intento anterior con otras columnas), se recrea --
    # stg es Bronze/truncate-reload, no hay perdida de datos que preservar.
...
def extraer_filas(as400_cur: pyodbc.Cursor, columnas: list) -> list:
    ...
    as400_cur.execute(
        f'SELECT {nombres} FROM {AS400_ESQUEMA_ORIGEN}.{AS400_TABLA_ORIGEN} WHERE "IID" = ?',
        FILTRO_TIPO_REGISTRO,
    )
...
def cargar_stg(jrm_conn, jrm_cur, columnas: list, filas: list, run_id: int) -> tuple:
    ...
    llave_idx = nombres_cols.index("IPROD") if "IPROD" in nombres_cols else None
...
def main() -> int:
    ...
    jrm_cur.fast_executemany = True  # evita una ida y vuelta de red por fila
