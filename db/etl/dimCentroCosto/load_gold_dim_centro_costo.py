"""
Gold: [int].dimCentroCosto -> dw.dimCentroCosto (SCD Tipo 1, nombres de negocio).

Todo ocurre dentro de JAREMAR -- el trabajo real lo hace
dw.usp_MergeDimCentroCosto; este script solo orquesta el ciclo de control
(registro de proceso, log de corrida, watermark).

Uso:
    python db/etl/dimCentroCosto/load_gold_dim_centro_costo.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "CentroCosto", "int", "int", "dimCentroCosto", "dw", "dimCentroCosto", "FULL",
    )
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC dw.usp_MergeDimCentroCosto @RunId = ?", run_id)
