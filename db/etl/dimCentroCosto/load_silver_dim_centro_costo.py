"""
Silver: stg.dimCentroCosto -> [int].dimCentroCosto (tipado, dedup por SVSGVL).

Todo ocurre dentro de JAREMAR -- el trabajo real lo hace
[int].usp_MergeDimCentroCosto; este script solo orquesta el ciclo de control
(registro de proceso, log de corrida, watermark).

Uso:
    python db/etl/dimCentroCosto/load_silver_dim_centro_costo.py [--env-file .env]
"""
import argparse
import datetime
import sys
...
def registrar_proceso(cur: pyodbc.Cursor) -> int:
    cur.execute(
        ...
        SELECT @pid;
        """,
        PROCESO, "CentroCosto", "stg", "stg", "dimCentroCosto", "int", "dimCentroCosto", "FULL",
    )
...
def main() -> int:
    ...
    try:
        cur.execute("EXEC [int].usp_MergeDimCentroCosto @RunId = ?", run_id)
