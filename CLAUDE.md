...
`python db/etl/dimPlanGeneracion/cargar_plan_generacion.py --archivo "<Reporte Ejecutivo>.xlsx" --version <V> [--fuente ...] [--anio 2026] [--dry-run] [--env-file ...]`. Lee la hoja `Plan_Diario_INV` (kWh planeados por **día e inversor**; el equipo de planta lo define en el propio Excel) y carga `stg` -> `[int]` -> `dw.dimPlanGeneracion` (migraciones 149-154). Es un solo script (no hay extract del AS400) y requiere `openpyxl`.

- **Grano y enlace:** una fila por (Fecha, CodigoInversor). `CodigoInversor` es el ID del Excel (`HARINA-1`, `JABON-3`...); en gold se enlaza con `dw.dimSmaDevices` por el ID que trae `DeviceName` (`HARINA-1 (SN ...)`), no por serial (hay seriales SMA repetidos en dos plantas), y como respaldo por el `SerialInversor` de la hoja `Inversores` cuando ese serial identifica un único dispositivo (ej. `JABON-2`, que el portal SMA nombra solo `SN 3006256658`). Otros fabricantes: por `dimDeviceCapacity.InverterId`. Un ID sin match se carga con las llaves en NULL y con aviso.
- **Validación antes de escribir:** cabecera `Periodo`, fechas válidas sin duplicados, valores numéricos >= 0; una celda vacía se omite (no equivale a 0); si hay errores no se escribe nada. Hoy la hoja cubre solo los 24 inversores SMA de PROALSA, REFINERÍA, PERFECTOR, HARINAS y JABÓN (no DETERGENTE, EDIF ADMIN ni MARGARINA) y llega hasta la fecha del reporte.
- **Versiones:** cada carga lleva `--version`; para cada (Fecha, Inversor) queda vigente (`EsVigente = 1`) la versión cargada más recientemente, y las fechas que la carga nueva no trae conservan la anterior. Recargar la misma versión es idempotente.
- **Resúmenes (se calculan en el DW, no se cargan):** `dw.vwPlanDiarioInversor`, `dw.vwPlanDiarioPI` (PI = planta de `dimSmaPlants`) y `dw.vwPlanDiarioPlantel` (OLEPSA = 'Km 13.5', INDASA = 'Km 15', por el prefijo del nombre de la planta).
- **Origen:** el propio Excel (definido a mano por planta). El Excel original (2026-08) tenía un error en las fórmulas de `Plan_Diario_PI` de MARGARINA, DETERGENTE y EDIF ADMIN (usaban el plantel equivocado); se corrigió, y no afecta esta carga porque esos PI no están en `Plan_Diario_INV`.
